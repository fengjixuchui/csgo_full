...
class Logger : public BaseStaticSystem,
			   public Singleton
{
...
public:
	Logger() : BaseStaticSystem("Logger", "Verbose - AlwaysFlush - AllowChat"),
			   Singleton(),
			   m_msg(),
...
#define SystemVerbose1(x)     \
	if (this->m_verbose >= 1) \
	g_Logger.Msg<MSG_VERBOSE1>(x, 1)
#define SystemVerbose2(x)     \
	if (this->m_verbose >= 2) \
	g_Logger.Msg<MSG_VERBOSE2>(x, 2)
...
#define LoggerAssert(expression)                             \
	if ((!(!!(expression))))                                 \
		Logger::SpewAssert(#expression, __FILE__, __LINE__); \
	Assert(expression)
#else
