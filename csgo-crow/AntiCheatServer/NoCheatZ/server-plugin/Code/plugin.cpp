...
void *CreateInterfaceInternal(char const *pName, int *pReturnCode)
{
	...
	else
	{
		...
		if (pReturnCode)
			*pReturnCode = SourceSdk::IFACE_FAILED;
		return nullptr;
	}

	if (pReturnCode)
		*pReturnCode = SourceSdk::IFACE_OK;
...
//---------------------------------------------------------------------------------
CNoCheatZPlugin::CNoCheatZPlugin() : SourceSdk::IServerPluginCallbacks(),
									 Singleton(),
...
void CNoCheatZPlugin::ClientActive(SourceSdk::edict_t *pEntity)
{
	...
	if (g_NczPlayerManager.GetPlayerCount(&filter_class) == 1)
		g_AutoTVRecord.SpawnTV();
}
